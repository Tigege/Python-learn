...
with tf.Session() as sess:
    ...
    for epoch in range(5):
        ...
        acc=sess.run(accracy,feed_dict={x:mnist.test.images,y:mnist.test.labels,keep_prob:1.0})
        print("Iter"+str(epoch)+",Testing Acc="+str(acc))
    print("test")
    test=sess.run(correct_prediction,feed_dict={x:mnist.test.images,y:mnist.test.labels,keep_prob:1.0})
