...
path = 'F:/datas/flower_photos/'  # 数据存放路径
...
# 4.placeholder 用于得到传递进来的真实的训练样本
# tf.float32 数据类型 创建输入数据的占位符，用于传递样本数据
...
def model(input_tensor, train,
          regularizer):  # 定义卷积神经网络模型model，三个变量分别是输入张量input_tensor，用于区分训练过程还是验证过程的train，正则项regularizer。
...
logits = model(x, True, regularizer)  # 模型调用，其中train参数为False，表示不是训练状态;
...
loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
                                                      labels=y_)  # 定义损失函数：计算预测标签logits和原始标签y_之间的sparse交叉熵
...
n_epoch = 20  # 样本训练次数，一个epoch代表用所有的数据训练一次
batch_size = 150  # batch_size代表使用梯度下降训练模型时候，即每次使用batch_size个数据来更新参数
...
for epoch in range(n_epoch):  # 总共有5个epoch，开始遍历每一层的epoch
    ...
    for x_train_a, y_train_a in minibatches(x_train, y_train, batch_size,
                                            shuffle=True):  # 利用minibatches函数从训练数据中以打乱顺序的方式提取批数据进行训练，批大小为64
        _, err, ac = sess.run([train_op, loss, acc], feed_dict={x: x_train_a,
                                                                y_: y_train_a})  # 运行train_op，loss，acc三个运算，利用feed_dict给占位符传输数据进行遍历训练
        train_loss += err;
        train_acc += ac;
    ...
    for x_val_a, y_val_a in minibatches(x_val, y_val, batch_size,
                                        shuffle=False):  # 利用minibatches函数从验证数据中以不打乱顺序的方式提取批数据进行验证，批大小为64
        err, ac = sess.run([loss, acc],
                           feed_dict={x: x_val_a, y_: y_val_a})  # 利用feed_dict给占位符传输数据进行遍历验证，运行train_op，loss，acc三个op运算
        val_loss += err;
        val_acc += ac;
        n_batch += 1  # 将每次迭代的损失值err，准确率ac，批次数n_batch进行累计
...
path1 = "F:/datas/flower_photos/daisy/5547758_eea9edfd54_n.jpg"
path2 = "F:/datas/flower_photos/dandelion/7355522_b66e5d3078_m.jpg"
path3 = "F:/datas/flower_photos/roses/394990940_7af082cf8d_n.jpg"
path4 = "F:/datas/flower_photos/sunflowers/6953297_8576bf4ea3.jpg"
path5 = "F:/datas/flower_photos/tulips/10791227_7168491604.jpg"
...
with tf.Session() as sess:  # 创建会话，用于执行已经定义的运算
    ...
    data.append(data4)
    data.append(data5)
    # 利用import_meta_graph函数直接加载之前已经持久化了的模型内容
    saver = tf.train.import_meta_graph('./model.ckpt.meta')
    # 利用restore函数加载已经训练好的模型，并利用tf.train.latest_checkpoint函数提取最近一次保存的模型
    ...
    output = []  # 定义空白列表output
    # 选择出预测矩阵每一行最大值的下标，并将字符串str当成有效的表达式来求值并返回计算结果，将其赋值给output
